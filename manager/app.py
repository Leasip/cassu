...
def admin_dashboard():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            elif action == 'decline_screenshot':
                player_id = request.form.get('player_id')
                casino_id = request.form.get('casino_id')
                cursor.execute('UPDATE player_casino SET status = ?, screenshot = NULL WHERE player_id = ? AND casino_id = ?', ('pending', player_id, casino_id))
                conn.commit()
    ...
    cursor.execute('''SELECT pc.player_id, pc.casino_id, pc.status, pc.screenshot, 
                      ...
                      WHERE pc.status = 'screenshot_uploaded' AND pc.screenshot IS NOT NULL
                   ''')
    ...
    conn.close()
    return render_template('admin_dashboard.html', players=players, casinos=casinos, pending_casinos=pending_casinos)
        ...
        if action == 'activate_casino':
            ...
            if not existing_entry:
                ...
                if active_players < casino[2]:
                    cursor.execute('INSERT INTO player_casino (player_id, casino_id, status, assigned_price) VALUES (?, ?, ?, ?)', (player_id, casino_id, 'pending', price))
                    conn.commit()
                    socketio.emit('update_dashboard')
            ...
            if 'screenshot' in request.files:
                ...
                if screenshot.filename != '':
                    ...
                    cursor.execute('UPDATE player_casino SET status = ?, screenshot = ? WHERE player_id = ? AND casino_id = ?', ('screenshot_uploaded', unique_filename, player_id, casino_id))
                    conn.commit()
                    socketio.emit('update_dashboard')
